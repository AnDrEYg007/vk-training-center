...
2.  **Автоматическое создание таблиц**:
    -   Сразу после создания файла, SQLAlchemy (ORM) читает все модели, определенные в `backend_python/models.py.txt`.
...
3.  **Автоматическое начальное заполнение (Seeding)**:
    -   После создания таблиц, код в `main.py.txt` проверяет, пуста ли таблица `projects`.
...
4.  **Автоматические миграции**:
    -   При последующих запусках сервера, скрипт `migrations.py.txt` может выполнять простые миграции, например, добавлять новые колонки, если они появились в моделях.
