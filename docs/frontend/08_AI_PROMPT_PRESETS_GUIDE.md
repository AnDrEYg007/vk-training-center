...
3.  **CRUD (`crud/ai_prompt_preset_crud.py`):**
...
4.  **Сервис и Роутер (`services/` и `routers/`):**
    -   Создан новый сервисный слой и роутер (`ai_prompt_preset_service.py` и `ai_presets.py`), которые предоставляют REST API для управления шаблонами.
