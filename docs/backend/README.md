...
Среда разработки сохраняет файлы Python с расширением `.py`. Перед началом работы их нужно переименовать.
...
find . -name "*.py" -exec sh -c 'mv "$1" "${1%.txt}"' _ {} \;

# Для Windows в PowerShell (рекурсивно для всех папок)
Get-ChildItem -Recurse -Filter *.py | ForEach-Object { Rename-Item -Path $_.FullName -NewName $_.Name.Replace(".py", ".py") }
```
