...
-   **Архитектурное улучшение: Рефакторинг сервисного слоя бэкенда**
    -   **Что сделано:** Проведен крупный рефакторинг бэкенда. Монолитный сервис `post_service.py.txt` был разделен на несколько небольших, сфокусированных модулей в соответствии с принципом единственной ответственности.
    -   **Как работает сейчас:**
        -   `post_retrieval_service.py.txt`: Отвечает только за **получение и обновление** данных из VK (например, `refresh_published_posts`).
        -   `post_actions_service.py.txt`: Содержит всю логику **действий** с постами (сохранение, удаление, публикация).
        -   `vk_callback_service.py.txt`: Инкапсулирует всю логику обработки **уведомлений от VK**.
        -   `post_helpers.py.txt`: Содержит вспомогательные функции (например, проверка конфликтов времени).
...
-   **Архитектурное улучшение: Рефакторинг и модульная структура сервиса VK API**
    -   **Что сделано:** Монолитный файл `vk_service.py.txt` на бэкенде был полностью реорганизован. Вся низкоуровневая логика взаимодействия с API ВКонтакте вынесена в отдельный пакет `services/vk_api/` с четким разделением ответственности.
    -   **Как работает сейчас:**
        1.  **`vk_api/api_client.py.txt`**: Содержит универсальную функцию `call_vk_api` для отправки запросов, включая надежную обработку ошибок и логику повторных попыток.
        2.  **`vk_api/formatters.py.txt`**: Инкапсулирует всю логику преобразования "сырых" данных от VK (посты, альбомы, фото) в наши внутренние структуры.
        3.  **`vk_api/utils.py.txt`**: Содержит все вспомогательные утилиты для работы с идентификаторами и URL.
        4.  **`services/vk_service.py.txt`**: Теперь выступает в роли **высокоуровневого сервиса-хаба**. Он содержит сложную бизнес-логику (например, 3-этапную загрузку фото) и импортирует низкоуровневые функции из `vk_api`, предоставляя их другим сервисам для сохранения **полной обратной совместимости**.
...
-   **Архитектурное улучшение: Рефакторинг и модульная структура схем данных (Pydantic)**
    -   **Что сделано:** Монолитный файл `schemas.py.txt` на бэкенде был разделен на логические модули в новой директории `schemas/`.
    -   **Как работает сейчас:**
        1.  **`base_models.py.txt`**: Содержит базовые модели данных (`Project`, `Post`, `Tag`), которые переиспользуются в разных частях API.
        2.  **`api_payloads.py.txt`**: Содержит все схемы, описывающие тела **входящих** запросов от фронтенда.
        3.  **`api_responses.py.txt`**: Содержит все схемы, описывающие тела **исходящих** ответов от бэкенда.
        4.  **`__init__.py.txt`**: Обеспечивает обратную совместимость, позволяя остальной части бэкенда продолжать использовать `import schemas` без изменений.
