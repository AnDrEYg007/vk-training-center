...
## Версия 1.74.0 - 2026-01-14

-   **Исправление ошибок: Универсальные конкурсы**
    -   **Исправление сбоя:** Устранена критическая ошибка (`TypeError`), возникавшая при выборе времени в настройках конкурса.
    -   **Сохранение данных:** Исправлена проблема, из-за которой описание конкурса и текст стартового поста не сохранялись корректно.
    -   **Интеграция с расписанием:** Реализовано корректное отображение системных постов универсальных конкурсов (Старт и Итоги) в календаре.
        -   Добавлена специальная цветовая индикация (Голубой для старта, Оранжевый для итогов).
        -   Реализовано модальное окно предпросмотра с информацией о конкурсе.
        -   Добавлена быстрая навигация из расписания к настройкам соответствующего конкурса.

## Версия 1.75.0 - 2026-01-15

-   **Новый функционал: Активные истории сообщества**
    -   **Вкладка "Статистика":** Добавлено разделение на "Архив" (из базы данных) и "Активные в VK".
    -   **Интеграция с API VK:** Реализовано отображение активных историй напрямую из ВКонтакте с информацией о просмотрах, типах и ссылках.
    -   **Синхронизация:** Истории, опубликованные через автоматизацию, помечаются специальным флагом.

-   **Техническое улучшение: Рефакторинг страницы автоматизации историй**
    -   **Оптимизация архитектуры:** Монолитный компонент `StoriesAutomationPage` разделен на хук логики (`useStoriesAutomation`) и презентационные компоненты (`StoriesSettingsView`, `StoriesStatsView`).
    -   **Типизация:** Все интерфейсы вынесены в отдельный файл `types.ts`.
    -   **Паттерн Container-View:** Внедрен паттерн разделения логики и представления для упрощения поддержки и расширения функционала.

-   **Улучшение бэкенда: Оптимизация статистики историй**
    -   **Точечное обновление:** Добавлена возможность обновления статистики конкретной истории без полной перезагрузки списка.
    -   **Новый эндпоинт:** Реализован метод `get_community_stories` для получения данных из VK API.
