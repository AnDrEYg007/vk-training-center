...
export const ClearRowModal: React.FC<ClearRowModalProps> = ({
    isOpen,
    onClose,
    onConfirm,
    project,
    ...
    // Изначально выбираем все поля
    const [selectedFieldIds, setSelectedFieldIds] = useState<Set<string>>(new Set(fields.map(f => f.id)));
    ...
    const handleConfirm = () => {
        onConfirm(Array.from(selectedFieldIds));
        onClose();
    };
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[70] p-4" onClick={onClose}>
            <div className="bg-white rounded-lg shadow-xl w-full max-w-lg animate-fade-in-up flex flex-col max-h-[90vh]" onClick={e => e.stopPropagation()}>
                <header className="p-4 border-b">
                    <h2 className="text-lg font-semibold text-gray-800">Очистка данных проекта</h2>
                    <p className="text-sm text-gray-500 mt-1">
                        Проект: <span className="font-medium text-gray-900">{project.name}</span>
                    </p>
                    <p className="text-xs text-gray-400 mt-1">Выберите столбцы, которые нужно очистить. Данные будут удалены локально и применятся после нажатия "Сохранить".</p>
                </header>
                ...
                >
                    <div className="flex items-center px-2 py-1 text-sm text-indigo-600 font-medium hover:text-indigo-800">
                        <input 
                            type="checkbox" 
                            checked={selectedFieldIds.size > 0 && selectedFieldIds.size === fields.length}
                            onChange={toggleAll}
                            onClick={(e) => e.stopPropagation()}
                            className="mr-2 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500 cursor-pointer pointer-events-none"
                        />
                        {selectedFieldIds.size === fields.length ? 'Снять выделение' : 'Выбрать все'}
                    </div>
                    <span className="ml-auto text-xs text-gray-500 px-2">Выбрано: {selectedFieldIds.size}</span>
                </div>
                        ...
                        {fields.map(field => (
                            ...
                            >
                                ...
                                <div className="ml-3 min-w-0">
                                    <p className="text-sm font-medium text-gray-700 truncate">{field.name}</p>
                                    {field.description && <p className="text-xs text-gray-400 truncate">{field.description}</p>}
                                </div>
                            </div>
                        ))}
                    </div>
                ...
                <footer className="p-4 border-t bg-gray-50 flex justify-end gap-3 rounded-b-lg">
                    ...
                    >
                        ...
                        Очистить ({selectedFieldIds.size})
                    </button>
                </footer>
            </div>
        </div>
