...
export const PublishedPost: React.FC<ContentProps> = ({ title }) => {
  ...
  return (
    <article className="prose prose-indigo max-w-none">
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-700">
              Опубликованные посты появляются в календаре автоматически после размещения на стене ВКонтакте. Приложение синхронизирует последние 20 постов через API и в реальном времени через Callback API. Вы можете редактировать или удалять их — изменения применяются к оригинальному посту на стене сообщества.
            </p>
          </div>
        </div>
      </div>
      ...
      <p className="!text-base !leading-relaxed !text-gray-700">
        При нажатии на кнопку «Редактировать» открывается модальное окно с формой редактирования поста. Вы можете изменить текст, добавить или удалить изображения, изменить вложения и время публикации.
      </p>
      <p className="!text-base !leading-relaxed !text-gray-700">
      ...
      <p className="!text-base !leading-relaxed !text-gray-700">
        При нажатии на «Копировать» открывается модальное окно с предзаполненной формой нового поста. По умолчанию копия создаётся как <strong>системный пост</strong>, который хранится внутри приложения.
      </p>
      <p className="!text-base !leading-relaxed !text-gray-700">
      ...
      <ol className="!text-base !leading-relaxed !text-gray-700">
        ...
        <li><strong>Перенос на другую дату:</strong> при отпускании карточки открывается модальное окно копирования с предзаполненной новой датой публикации</li>
      ...
      <h3 className="!text-xl !font-semibold !text-gray-800 !mt-8">2. Реалтайм обновления (Callback API)</h3>
      <p className="!text-base !leading-relaxed !text-gray-700">
        Приложение использует Callback API ВКонтакте для получения уведомлений о новых публикациях. Когда пост публикуется на стене сообщества (сразу или из отложенных записей), приложение получает событие <code>wall_post_new</code>.
      </p>
      <p className="!text-base !leading-relaxed !text-gray-700">
      ...
      <details className="bg-gray-50 border border-gray-200 rounded-lg p-4 mb-4">
        ...
        <p className="mt-2 text-gray-700">
          Периодическая синхронизация через <code>wall.get</code> происходит по расписанию, установленному на сервере (обычно раз в несколько минут). Реалтайм-обновления через Callback API срабатывают мгновенно при публикации нового поста. Вы также можете принудительно обновить кэш, нажав кнопку обновления в интерфейсе календаря.
        </p>
      </details>
      ...
      <div className="bg-gray-100 border border-gray-300 rounded-lg p-6 my-8">
        ...
        <ul className="space-y-2 text-gray-700">
          ...
          <li className="flex items-start">
            <span className="text-indigo-600 mr-2">•</span>
            Синхронизация происходит автоматически через метод API <code>wall.get</code> (последние 20 постов) и в реальном времени через Callback API при новых публикациях.
          </li>
          <li className="flex items-start">
            <span className="text-indigo-600 mr-2">•</span>
