...
interface GeneralWinnersTabProps {
    contestId?: string;
}

export const GeneralWinnersTab: React.FC<GeneralWinnersTabProps> = ({ contestId }) => {
    ...
    const loadWinners = useCallback(async () => {
        ...
        setIsLoading(true);
        try {
            const logs = await api.getGeneralContestWinners(contestId);
            setWinners(logs);
        } catch (error) {
            console.error("Failed to load general contest winners:", error);
        } finally {
            setIsLoading(false);
        }
    }, [contestId]);
        ...
        <div className="opacity-0 animate-fade-in-up h-full flex flex-col">
            <div className="bg-white rounded-lg shadow border border-gray-200 overflow-hidden flex-grow flex flex-col">
                <div className="p-4 border-b border-gray-200 bg-amber-50 flex items-center justify-between">
                    ...
                    <button 
                        onClick={loadWinners}
                        className="px-3 py-1.5 text-sm bg-white border rounded hover:bg-amber-100 text-amber-700 border-amber-200"
                    >
                        Обновить
                    </button>
                </div>
                                ...
                                {winners.map((winner) => (
                                    <tr key={winner.id} className="hover:bg-amber-50/30 transition-colors">
                                        <td className="px-6 py-4 text-gray-500 whitespace-nowrap">
                                            {winner.created_at ? new Date(winner.created_at).toLocaleString('ru-RU') : '-'}
                                        </td>
                                        ...
                                        <td className="px-6 py-4 text-right">
                                            {winner.status === 'sent' ? (
                                                ...
                                                <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800" title={winner.error_details || 'ЛС закрыто или ошибка отправки'}>
                                                    Комментарий/Ошибка
                                                </span>
                                            )}
                                        </td>
